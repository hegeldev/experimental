...
/**
 * Unit tests for Session — version parsing, command building, and reset().
 * These test package-private methods without spawning a subprocess.
 */
class SessionTest {
    ...
    /**
     * sendRequest() throws IOException → lines 127-129.
     * Use an OutputStream that always fails so the packet write fails.
     */
    @Test
    void connectAndHandshakeSendFailure() throws Exception {
        ...
        OutputStream failOut = new OutputStream() {
            @Override public void write(int b) throws IOException { throw new IOException("write failed"); }
            @Override public void write(byte[] b, int off, int len) throws IOException { throw new IOException("write failed"); }
        };

        RuntimeException ex = assertThrows(RuntimeException.class, () ->
            Session.connectAndHandshake(emptyIn, failOut, null)
        );
        assertTrue(ex.getMessage().contains("handshake") || ex.getCause() instanceof IOException,
                   "Unexpected: " + ex.getMessage());
    }

    /**
     * sendRequest() succeeds but receiveReply() throws → lines 135-137.
     * Server reads the handshake packet, then closes its output (EOF for client).
     */
    @Test
    void connectAndHandshakeReceiveFailure() throws Exception {
        ...
        RuntimeException ex = assertThrows(RuntimeException.class, () ->
            Session.connectAndHandshake(clientIn, clientOut, null)
        );
        assertTrue(ex.getMessage().contains("handshake") || ex.getCause() instanceof IOException,
                   "Unexpected: " + ex.getMessage());
    ...
    void connectAndHandshakeBadResponse() throws Exception {
        ...
        assertTrue(ex.getMessage().contains("Bad handshake"),
                   "Unexpected: " + ex.getMessage());
    ...
    void connectAndHandshakeWrongVersion() throws Exception {
        ...
        assertTrue(ex.getMessage().contains("protocol version") || ex.getMessage().contains("supports"),
                   "Unexpected: " + ex.getMessage());
    ...
    /**
     * init() fails to start process → lines 100-103.
     * Also covers serverLogFile() IOException catch (line 197) via testServerLogDir.
     */
    @Test
