...
/**
 * Unit tests for ServerDataSource error paths. Uses piped streams to simulate a fake hegel-core
 * server.
 */
class ServerDataSourceTest {
  ...
  void generateIOExceptionOnSendRequest() throws Exception {
    try (FakeServer fs = new FakeServer()) {
      ...
      StopTestException ex = assertThrows(StopTestException.class, () -> ds.generate(schema));
  ...
  void generateFlakyStrategyError() throws Exception {
    try (FakeServer fs = new FakeServer()) {
      ...
      serverThread.join(3000);
      assertTrue(ds.testAborted());
      assertTrue(ex.getMessage().contains("flaky"), "Unexpected: " + ex.getMessage());
    }
  }
  ...
  void generateGenericServerError() throws Exception {
    try (FakeServer fs = new FakeServer()) {
      ...
      RuntimeException ex = assertThrows(RuntimeException.class, () -> ds.generate(schema));
      serverThread.join(3000);
      assertFalse(ex instanceof StopTestException);
      assertTrue(ex.getMessage().contains("Server error"), "Unexpected: " + ex.getMessage());
    }
  }
