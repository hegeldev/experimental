 ...
 *
 * <p>Conformance test binaries receive JSON params as a command-line argument, run a Hegel test,
 * and write metrics (one JSON line per test case) to {@code CONFORMANCE_METRICS_FILE}.
 */
public final class ConformanceHelper {
  ...
  /**
   * Parse the JSON params argument from {@code args[0]}. Returns an empty ObjectNode if args is
   * empty or params is empty.
   */
  ...
  /**
   * Return the number of test cases from the {@code CONFORMANCE_TEST_CASES} env var. Defaults to
   * 50.
   */
  ...
  /**
   * Write a metrics object to the metrics file (one JSON line). Must be called inside the Hegel
   * test body for each test case.
   */
