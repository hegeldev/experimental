 ...
 *
 * <ul>
 *   <li>{@code basic}: Uses the basic (schema-composition) path.
 *   <li>{@code non_basic}: Forces the compositional path by using a no-op filter.
 * </ul>
 */
public class TestMaps {
  public static void main(String[] args) {
    ...
    Generator<Long> valueGenerator =
        "non_basic".equals(mode) ? valueGen.filter(v -> true) : valueGen;
    ...
    Hegel.test(
        ...
        tc -> {
          ...
          if ("string".equals(keyType)) {
            ...
            Generator<String> keyGenerator =
                "non_basic".equals(mode) ? keyGen.filter(s -> true) : keyGen;
            var mapGen =
                Generators.maps(keyGenerator, valueGenerator).minSize(minSize).maxSize(maxSize);
            Map<String, Long> map = tc.draw(mapGen);

            metrics.put("size", map.size());
            if (!map.isEmpty()) {
              metrics.put(
                  "min_value", map.values().stream().mapToLong(Long::longValue).min().orElse(0));
              metrics.put(
                  "max_value", map.values().stream().mapToLong(Long::longValue).max().orElse(0));
            } else {
            ...
            Generator<Long> keyGenerator =
                "non_basic".equals(mode) ? keyGen.filter(k -> true) : keyGen;
            var mapGen =
                Generators.maps(keyGenerator, valueGenerator).minSize(minSize).maxSize(maxSize);
            ...
            if (!map.isEmpty()) {
              metrics.put(
                  "min_key", map.keySet().stream().mapToLong(Long::longValue).min().orElse(0));
              metrics.put(
                  "max_key", map.keySet().stream().mapToLong(Long::longValue).max().orElse(0));
              metrics.put(
                  "min_value", map.values().stream().mapToLong(Long::longValue).min().orElse(0));
              metrics.put(
                  "max_value", map.values().stream().mapToLong(Long::longValue).max().orElse(0));
