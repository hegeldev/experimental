 ...
 *
 * <p>These utilities use Hegel to test Hegel: they run property tests internally
 * and provide higher-level assertions about what generators can and cannot produce.
 *
 * <h2>Usage</h2>
 * <pre>{@code
...
public final class HegelTestUtils {
     ...
     *
     * <p>Runs a property test that draws from {@code gen} and checks {@code pred}.
     * Throws {@link AssertionError} (with the minimal failing value) if any value
     * does not satisfy the predicate.
     */
    public static <T> void assertAllExamples(Generator<T> gen, Predicate<T> pred) {
        Hegel.test("assertAllExamples", tc -> {
            T val = tc.draw(gen);
            if (!pred.test(val)) {
     ...
     *
     * <p>Throws {@link AssertionError} (with the minimal satisfying value) if any
     * generated value satisfies {@code cond}.
     */
    public static <T> void assertNoExamples(Generator<T> gen, Predicate<T> cond) {
        Hegel.test("assertNoExamples", tc -> {
            T val = tc.draw(gen);
            if (cond.test(val)) {
     ...
     *
     * <p>The returned value is the minimal (most-shrunk) example satisfying the
     * condition. Throws {@link AssertionError} if no matching value can be found.
     */
    public static <T> T findAny(Generator<T> gen, Predicate<T> cond) {
        AtomicReference<T> result = new AtomicReference<>();
        try {
            Hegel.test("findAny", tc -> {
                T val = tc.draw(gen);
                if (cond.test(val)) {
                    result.set(val);
     ...
     *
     * <p>Useful for verifying that Hegel's shrinking finds the true minimum.
     * For example, {@code minimal(integers(), x -> x >= 100)} should return
     * exactly {@code 100L}.
     *
