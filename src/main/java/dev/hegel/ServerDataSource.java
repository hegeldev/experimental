...
/**
 * {@link DataSource} that communicates with the hegel-core server over
 * a multiplexed {@link Stream}.
 */
public class ServerDataSource implements DataSource {
    ...
    private JsonNode sendRequest(String command, ObjectNode extra) throws StopTestException {
        ...
        if (decoded.isObject() && decoded.has("error")) {
            ...
            if (errorMsg.contains("overflow") || errorMsg.contains("StopTest")
                    || errorType.contains("overflow") || errorType.contains("StopTest")) {
                stream.markClosed();
                aborted = true;
