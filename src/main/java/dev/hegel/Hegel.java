 ...
 * }</pre>
 *
 * <p>This spawns a hegel-core server on first use and reuses it for all tests in the JVM process.
 */
public class Hegel {
   ...
   *
   * <p>Connects to the hegel server (spawning it on first use), runs the configured number of test
   * cases, and throws {@link AssertionError} if any test case finds a counterexample.
   */
  public void run() {
  ...
  void checkResults(JsonNode resultData, AssertionError lastFailure) {
    ...
    if (resultData.has("health_check_failure")
        && !resultData.get("health_check_failure").isNull()) {
      throw new AssertionError(
          "Health check failure:\n" + resultData.get("health_check_failure").asText());
    }
    if (resultData.has("flaky") && !resultData.get("flaky").isNull()) {
      throw new AssertionError("Flaky test detected: " + resultData.get("flaky").asText());
    }
