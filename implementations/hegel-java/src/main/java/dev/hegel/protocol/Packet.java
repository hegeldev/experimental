 ...
 *
 * <ul>
 *   <li>4 bytes: magic {@code 0x4845474C} ("HEGL")
 *   <li>4 bytes: CRC32 of the header (checksum field zeroed) + payload
 *   <li>4 bytes: stream ID
 *   <li>4 bytes: message ID (high bit set → reply)
 *   <li>4 bytes: payload length
 *   <li>N bytes: CBOR payload
 *   <li>1 byte: terminator {@code 0x0A}
 * </ul>
 */
public record Packet(int streamId, int messageId, boolean isReply, byte[] payload) {
  ...
  public static Packet read(InputStream in) throws IOException {
    ...
    if (terminator != (TERMINATOR & 0xFF)) {
      throw new IOException(
          String.format("Invalid terminator: expected 0x0A, got 0x%02X", terminator));
    }
    ...
    if ((int) computed != checksum) {
      throw new IOException(
          String.format("CRC32 mismatch: expected 0x%08X, got 0x%08X", checksum, (int) computed));
    }
